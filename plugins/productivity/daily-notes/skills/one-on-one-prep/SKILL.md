...
This skill reads per-person files under `{contacts_folder}/<Name>/`. Read the "Daily Notes Plugin Profile" section from your CLAUDE.md context.
...
8. **Present the prep** (plain markdown):
   ```
...
9. **Obsidian profile check**: If `obsidian: true` is set, render every section as a callout and wrap the person's name in `[[<Name>]]` wikilinks:
...
10. **Append to next 1:1 note?**: Ask the user if they'd like to append this prep block to today's 1:1 meeting note. If yes:
...
- Read-only by default. Only write when the user explicitly confirms step 10.
